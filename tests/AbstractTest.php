...
abstract class AbstractTest extends WebTestCase
{
    ...
    /**
     * Последовательно отправляет формы с неверными полями и проверяет тексты ошибок
     * @param $submitButton
     * @param array $fieldData Словарь с данными о форме. Пример:
     * [
     *  'validData' => [
     *      'lesson[name]' => 'validName'
     *  ],
     *  'invalidData' => [
     *      'lesson[name]' => [
     *          '' => 'Название не может быть пустым',
     *          str_copy('t', 260) => 'Название должно быть короче 255 символов'
     *      ]
     *  ]
     * ]
     * @return void
     */
    public function assertFormFieldsValidation($submitButton, array $fieldData)
    {
        //проходим по полям с неправильными данными
        foreach ($fieldData['invalidData'] as $key => $values) {
            //проходим по неправильным данным и кодам ошибок
            foreach ($values as $val => $error) {
                //создаём новую форму
                $form = $submitButton->form();
                //заполняем форму правильными данными
                foreach ($fieldData['validData'] as $field => $value) {
                    $form[$field] = $value;
                }
                //включаем в форму поле с неправильными данными
                $form[$key] = $val;

                self::getClient()->submit($form);
                $this->assertResponseCode(Response::HTTP_UNPROCESSABLE_ENTITY);
                $this->assertSelectorTextContains('.invalid-feedback.d-block', $error);
            }
        }
    }
}
